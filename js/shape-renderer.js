...
/**
 * Central utility function to draw any shape type - OPTIMIZED with Unified Shape Handler
 * @param {CanvasRenderingContext2D} ctx - The canvas context
...
function drawShape(ctx, shape, zoom, shapeIndex) {
    ...
    if (window.shapeHandler) {
        const result = window.shapeHandler.execute('render', shape.type, ctx, shape, zoom, shapeIndex);
        if (result !== null) {
    ...
    switch(shape.type) {
        ...
        case 'text':
            if (shape.x !== undefined && shape.y !== undefined) {
                ...
                // Text corner handles
                drawHandleLocal(shape.x, shape.y, 'endpoint'); // Baseline start (insertion point)
                drawHandleLocal(shape.x + textWidth, shape.y, 'midpoint'); // Baseline end
                drawHandleLocal(shape.x, shape.y - textHeight, 'midpoint'); // Top left
                drawHandleLocal(shape.x + textWidth, shape.y - textHeight, 'midpoint'); // Top right
            }
            break;
