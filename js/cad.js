...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Initialize lineweight selector
...
let currentLineWeight = "byLayer"; // Default to byLayer
...
function getEffectiveLineweight(shape) {
    if (shape.lineweight === 'byLayer' || shape.lineweight === undefined) {
...
// Function to zoom to fit all objects
function zoomToFit() {
    if (shapes.length === 0) {
...
function setMode(m) {
    ...
    const statusMessages = {
        ...
        'polyline': 'Step 1/?: Click first point for polyline',
        ...
        'spline': 'Step 1/?: Click first point for spline',
...
function drawShapeOutline(ctx, shape) {
    // Draw outline around shape for selection highlighting
    switch(shape.type) {
        case 'line':
            ctx.beginPath();
...
function pasteShapes(x, y) {
    ...
    copiedShapes.forEach(shape => {
        ...
        if (newShape.type === 'line') {
            ...
            newShape.x2 += offsetX;
            newShape.y2 += offsetY;
        } else if (newShape.type === 'circle' || newShape.type === 'arc') {
            newShape.cx += offsetX;
            newShape.cy += offsetY;
...
function handlePolylineMode(x, y, e) {
    ...
    if (polylinePoints.length === 0) {
        ...
        updateHelpBar('Step 2/?: Click next point, type length + Enter, or double-click/Enter/Escape to finish');
        ...
        updateHelpBar(`Step ${polylinePoints.length + 1}/?: Click next point, type length + Enter, or double-click/Enter/Escape to finish`);
...
function handleSplineMode(x, y, e) {
    ...
    if (splinePoints.length === 0) {
        ...
        // Show length input immediately like in line mode
        showLengthInput(e.offsetX, e.offsetY);
        updateHelpBar('Step 2/?: Click next point, enter length, or press Enter to finish spline');
        ...
        updateHelpBar(`Step ${splinePoints.length + 1}/?: Click next point, enter length, double-click or press Enter to finish`);
...
canvas.addEventListener('mousedown', (e) => {
    if (e.button === 1) { // Middle mouse button
        ...
        const currentTime = Date.now();
        if (currentTime - lastMiddleClickTime < 400) { // Double click detected (increased timeout)
            zoomToFit();
            redraw();
...
canvas.addEventListener('mousemove', (e) => {
    ...
    else if ((mode === 'line' || mode === 'circle' || mode === 'polygon') && isDrawing) {
        [x, y] = applyOrtho(x, y, startX, startY);
        [previewX, previewY] = [x, y];
        
        // Update length input position and direction for line and circle modes
        if (mode === 'line') {
...
document.addEventListener('keydown', (e) => {
    ...
    if (e.key === 'Enter' && mode === 'polyline' && polylinePoints.length > 1) {
        e.preventDefault();
        ...
        updateHelpBar('Polyline completed! Select another tool or object.');
        polylinePoints = [];
        polylinePreviewActive = false; // Reset preview flag
        setMode('select');
        redraw();
    ...
    if (e.key === 'Enter' && mode === 'spline' && splinePoints.length > 1) {
        e.preventDefault();
...
function handleLengthInput(length) {
    if (mode === 'line' && isDrawing && length > 0) {
        ...
        const [mouseWorldX, mouseWorldY] = screenToWorld(currentMouseX, currentMouseY);
        let dx = mouseWorldX - lastPoint.x;
        let dy = mouseWorldY - lastPoint.y;
        ...
        // Keep drawing mode active for next point
        updateLengthInputPosition(currentMouseX, currentMouseY);
        lengthInput.value = ''; // Clear field for new input
        lengthInput.focus();
