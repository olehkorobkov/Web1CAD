...
// clearSelection and selectAll are now in /core/selection.js
// deleteSelected, copySelected, pasteShapes are now in /core/shapes.js
...
lengthInput.addEventListener('keydown', (e) => {
    ...
    if (mode === 'polygon' && polygonStep === 1) {
        if (e.key === 'Enter') {
            ...
            if (!isNaN(sides) && sides >= 3 && sides <= 99) {
    ...
    // Allow Tab to show radius input for polygon step 2
    if (mode === 'polygon' && polygonStep === 2 && e.key === 'Tab') {
        e.preventDefault();
        if (!isLengthInputActive) {
            showLengthInput(currentMouseX, currentMouseY);
            updateLengthInputLabel('Radius:');
            lengthInput.focus();
        }
        return;
    }
    ...
    setTimeout(() => {
        if (isLengthInputActive && 
            ((mode === 'line' && isDrawing) || 
             ...
             (mode === 'polygon' && (polygonStep === 1 || polygonStep === 2)) ||
...
function handlePolygonLengthInput(value) {
    if (polygonStep === 1) {
        ...
        if (sides >= 3 && sides <= 99) {
            ...
            updateHelpBar('Step 3/3: Drag to set radius or press Tab to enter radius value');
            addToHistory(`Number of sides set to ${polygonSides} - ${polygonRadiusType} polygon selected`);
        } else {
            addToHistory('Number of sides must be between 3 and 99', 'error');
            return false;
        }
...
// window.setMode is now in /core/state.js
window.undo = undo;
window.redo = redo;
